...
#else
    LAPACK_SLATRS( &uplo, &lapack_option< Trans >::value, &lapack_option<
            Diag >::value, &normin, &n, a, &lda, x, &scale, cnorm, &info ,1 ,1 ,1 ,1 );
#endif
    return info;
}
...
struct latrs_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const char uplo, const char normin,
        ...
        static_assert( (bindings::is_mutable< VectorCNORM >::value) );
        assert( bindings::size(x) >= bindings::size_column_op(a,
                trans()) );
        assert( bindings::size_column_op(a, trans()) >= 0 );
        assert( bindings::size_minor(a) == 1 ||
                bindings::stride_minor(a) == 1 );
        assert( bindings::stride_major(a) >= ?MAX );
...
//
template< typename Value >
struct latrs_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
