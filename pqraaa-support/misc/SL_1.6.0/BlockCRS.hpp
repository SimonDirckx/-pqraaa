...
template< typename T >
class BlockCRS: public BlockOrderer< T > {
	protected:
		...
		virtual void in_readout  ( const unsigned long int index ) {
			...
			for( ; it != this->items[ index ].end(); ++it ) {
				...
				if( this->upper_vertical( index, *it ) )
					...
					cur4.push_back( *it );
				else if( this->lower_vertical( index, *it ) )
					cur5.push_back( *it );
				else
					rep.push_back( *it );
			}
			if( cur1.size() + cur2.size() + cur3.size() + cur4.size() + cur5.size() > 0 )
