...
#ifndef cork_mor_cork_hpp
#define cork_mor_cork_hpp
...
namespace CORK { namespace eigs {
 ...
 info cork( Linearization& linearization, EigSelector const& eigenvalue_selector, ShiftGenerator & shift_generator, InitialVector const& initial_vector, Quadruple& quadruple,
   ...
   /*  if (n_locked>0) {
     ...
     {
       glas2::range range_o_l( n_locked, old_n_locked ) ;
       glas2::range range_o_k_l( old_n_locked, quadruple.k() ) ;
       auto schur_vec_X_k_l = schur_vectors_X( range_o_k_l, range_o_k_l ) ;
       auto schur_vec_Y_l = schur_vectors_Y( range_o_l, range_o_l ) ;
       glas2::matrix<value_type> temp( range_o_l.size(), range_o_k_l.size() ) ;

       temp = multiply( quadruple.H( range_o_l, range_k_l ), schur_vec_X_k_l ) ;
       quadruple.H( range_o_l, range_k_l ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;

       temp = multiply( quadruple.K( range_o_l, range_o_k_l ), schur_vec_X_k_l ) ;
       quadruple.K( range_o_l, range_o_k_l ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;
     }
     //std::cout << "order_unlocked " << order_unlocked(glas2::range(0,keep_unlocked)) << std::endl ;

     // Reorder the unlocked part
     {
       assert (norm_fro( schur_vectors_X( range_l, range_k_l ) )==0.0 ) ;
       assert (norm_fro( schur_vectors_Y( range_l, range_k_l ) )==0.0 ) ;
     ...
     {
       // BUG HERE.
       //auto schur_vec_X_k_l = schur_vectors_X( range_keep_l, range_keep_l ) ;
       auto schur_vec_Y_l = schur_vectors_Y( range_l, range_l ) ;
       auto schur_vec_X_k_l = schur_vectors_X( range_k_l, range_keep_l ) ;
       glas2::matrix<value_type> temp( range_l.size(), range_keep_l.size() ) ;
