...
/*---------------------------------------------------------------------------*
 * Complex version of ilutpC
 *---------------------------------------------------------------------------*
 * 
 *                                                                           *
 * Report bugs / send comments to: saad@cs.umn.edu, dosei@cs.umn.edu           *
 *---------------------------------------------------------------------------*/
...
| w         = complex work array of length n. 
|----------------------------------------------------------------------- 
...
|--------------------------------------------------------------------*/
   ...
   double  xmax, xmax0, t1, sgny;
   complex double *rowm,t, s, fact, *w=NULL, tmp; 
   int dec, decnt=0;
   int zqsplitC(complex double *, int *, int, int);
   int zcpermC(csptr, int *);
#ifdef ILUTIME
   ...
   if (rmax > 0) {
      ...
      w = (complex double *) Malloc(rmax*sizeof(complex double), "ilutpC:2" );
...
|--------------------------------------------------------------------*/
	 ...
	 fact = w[jj] * rowm[0];
	 if ( cabs(fact) > drop5 ) {    /*  DROPPING IN L  */
#ifdef ILUTIME
msg_timer_start(&t25);
#endif
...
msg_timer_start(&t27);
      ...
      if (lenl > len)
         zqsplitC(w, jw, lenl, len);
/*
  printf("  row %d   length of L = %d",ii,len);
*/
      if (len > 0) {
	 ilusch->L->ja[ii] = (int *) Malloc(len*sizeof(int), "ilutpC:5" ); 
	 ilusch->L->ma[ii] = (complex double *) Malloc(len*sizeof(complex double), "ilutpC:6"); 
	 memcpy(ilusch->L->ma[ii], w, len*sizeof(complex double));
	 for (j=0; j<len; j++)
...
msg_timer_start(&t28);
	 ...
	 if ( cabs(w[ii+j]) > drop6*tnorm ) {
      ...
      if (lenu > len+1)
         zqsplitC(&w[ii+1], &jw[ii+1], lenu-1, len);
      ilusch->U->ma[ii] = (complex double *) Malloc(len*sizeof(complex double), "ilutpC:7" ); 
 ...
 */
      if (w[ii] == 0.0)  w[ii] = (0.0001+drop6)*tnorm + 0.0*I; 
/*-------------------- complex shift */      
    t = drop6*0.1;
/* Don't remove the two explicit casts in the following line:
 * It's a workaround for a GCC 4.1/4.2 bug */
    sgny = cimag(w[ii]) <=0 ? (double)-t : (double)t;
    w[ii] = w[ii]+sgny*I;

    ilusch->U->ma[ii][0] = 1.0 / w[ii];
    ilusch->U->ja[ii][0] = ii;

    memcpy(&ilusch->U->ma[ii][1], &w[ii+1], (len-1)*sizeof(complex double));
    lenu = 0;
      for (k=ii+1; k<ii+len; k++)
...
msg_timer_stop(&t28);
   ...
   zcpermC(ilusch->U, iprev);
   zcpermC(ilusch->L, iprev);
...
|--------------------------------------------------------------------*/
   ...
   double tnorm, drop5=droptol[5], drop6=droptol[6];
   int rowz, *rowj;
   complex double t, s, fact, *w=NULL, *rowm;
   int zqsplitC(complex double *, int *, int, int);
   ilusch->n = rmax = amat->n;
   if (rmax == 0) return(0); 
   if (rmax > 0) {
      jw = (int *) Malloc(rmax*sizeof(int), "ilutD:1" );
      w = (complex double *) Malloc(rmax*sizeof(complex double), "ilutD:2" );
...
|--------------------------------------------------------------------*/
      ...
      if (len > lenl)
         zqsplitC(w, jw, len, lenl);
      if (len > 0) {
	 ilusch->L->ja[ii] = (int *) Malloc(lenl*sizeof(int), "ilutD:4" ); 
	 ilusch->L->ma[ii] = (complex double *) Malloc(lenl*sizeof(complex double), "ilutD:5"); 
	 memcpy(ilusch->L->ja[ii], jw, lenl*sizeof(int));
	 memcpy(ilusch->L->ma[ii], w, lenl*sizeof(complex double));
      }
/*---------------------------------------------------------------------
...
|--------------------------------------------------------------------*/
	 ...
	 if ( cabs(w[ii+j]) > drop6*tnorm ) {
      ...
      jpos = lenu-1;
      if (len > jpos)
         zqsplitC(w, jw, len, jpos);
      ilusch->U->ma[ii] = (complex double *) Malloc(lenu*sizeof(complex double), "ilutD:6" ); 
      ilusch->U->ja[ii] = (int *) Malloc(lenu*sizeof(int), "ilutD:7" ); 
      if (cabs(t) == 0.0) t=(0.0001+drop6)*tnorm + 0.0*I;
...
|--------------------------------------------------------------------*/
      ...
      memcpy(&ilusch->U->ma[ii][1], w, jpos*sizeof(complex double));
   }
/*---------------------------------------------------------------------
|     end main loop - now do clean up
|--------------------------------------------------------------------*/
