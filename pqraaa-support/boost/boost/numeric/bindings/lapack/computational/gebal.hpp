...
#else
    LAPACK_SGEBAL( &job, &n, a, &lda, &ilo, &ihi, scale, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_DGEBAL( &job, &n, a, &lda, &ilo, &ihi, scale, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CGEBAL( &job, &n, a, &lda, &ilo, &ihi, scale, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_ZGEBAL( &job, &n, a, &lda, &ilo, &ihi, scale, &info ,1 );
#endif
    return info;
}
...
struct gebal_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct gebal_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
