...
#ifndef cork_linearization_cork_linearization_solve_qr_handle_hpp
#define cork_linearization_cork_linearization_solve_qr_handle_hpp
...
namespace CORK { namespace linearization {
  ...
  class cork_linearization_solve_handle
  {
    ...
    public:
      ...
      , MsN_q_( poly.size(), poly.size()-1 )
      , MsN_r_( poly.size(), poly.size()-1 )
      {
    ...
    public:
      void shift( value_type s ) {
        ...
        try {
          ...
          // QR factorization of M-sN
          glas2::matrix<value_type> temp( basis_handle_.size()-1, basis_handle_.size() ) ;
          basis_handle_.fill_M( temp ) ;
          MsN_r = trans(temp) ;
          basis_handle_.fill_N( temp ) ;
          MsN_r -= s * trans(temp) ;
          lapack::qr( MsN_r, MsN_q ) ;
    ...
    public:
      ...
      //std::cout << "Zr solve MN " << Zr << std::endl ;
        ...
        value_type s = shift() ;
        matrix_iterator_.schedule_b_0( coefficient_matrices_4cork_/*.accumulator()*/, [&Zr,&s] (auto i) { return s * Zr(i-1, glas2::all()) ; } ) ;

        // Compute A-terms for the right-hand side.
        matrix_iterator_.schedule_a_0( coefficient_matrices_4cork_/*.accumulator()*/, [&Zr] (auto i) { return -Zr(i-1, glas2::all()) ; } ) ;
        CORK::timer timer ;
        timer.tic() ;
