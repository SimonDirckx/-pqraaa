...
#include <cork/linearization/barycentric_rational_strong.hpp>
...
namespace CORK {
  ...
  */
  ...
  decltype (auto) aaa_cork( NEP const& problem, Domain const& aaa_domain, EigenvalueSelector const& eig_selector, Options const& options ) {
    ...
    typedef typename std::conditional< options::has_option<options::handle_as_real_problem_tag,Options>::value, real_type, value_type>::type  krylov_value_type ;
    ...
    auto [PEP_aaa, aaa_info] = CORK::matrix_valued_function::sv_aaa( nep_aaa, aaa_domain, norm_est, options ) ;

    if (options::value_of<options::test_approximation_error>(options)) {
      matrix_valued_function::difference ( nep_aaa, PEP_aaa, aaa_domain, options ) ;
    }
    ...
    CORK::krylov::cork_quadruple<krylov_value_type,decltype(options)> quadruple( linearization, n_wanted, options ) ;
