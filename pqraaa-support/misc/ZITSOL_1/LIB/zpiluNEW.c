...
|--------------------------------------------------------------------*/
     for (k=0; k<lrowz; k++)
       if (cabs(lrowm[k]) != 0.0) goto label41;
     goto label9996;
/*---------------------------------------------------------------------
...
|--------------------------------------------------------------------*/
       ...
       fact = w[jj] * lrowm[0];
       if (cabs(fact) > drop0 ) {   /*   DROPPING IN L   */
...
|--------------------------------------------------------------------*/
     ...
     if (len > 0) {
       ...
       amat->L->ma[ii] = (complex double *) Malloc(lenl*sizeof(complex double), "pilu:11" ); 
       memcpy(amat->L->ja[ii], jw, lenl*sizeof(int));
       memcpy(amat->L->ma[ii], w, lenl*sizeof(complex double));
      }
/*---------------------------------------------------------------------
...
|--------------------------------------------------------------------*/
     ...
     amat->U->ma[ii] = (complex double *) Malloc(lenu*sizeof(complex double), "pilu:12" );
     amat->U->ja[ii] = (int *) Malloc(lenu*sizeof(int), "pilu:13" );
     if (cabs(t) == 0.0) t=(0.0001+drop1) + 0.0*I;
/*-------------------- add complex shift */
     sgny = drop1; 
     sgny = cimag(t) <= 0 ? -sgny : sgny ;
     t = t+sgny*I;
...
|--------------------------------------------------------------------*/
     ...
     memcpy(&amat->U->ma[ii][1], w, jpos*sizeof(complex double));
/*---------------------------------------------------------------------
|     copy  L^{-1} F
|--------------------------------------------------------------------*/
     len = 0;
     for (j=0; j<len2; j++) {
       if ( cabs(w2[j]) > drop2*tnorm ) {
	 w[len] = w2[j];
	 jw[len] = jw2[j];
	 len++;
       }
     ...
     if (lenu < len)
       zqsplitC(w, jw, len, lenu);
     lflen[ii] = lenu;
     
     if (lenu > 0) {
       lfja[ii]  = (int *) Malloc(lenu*sizeof(int), "pilu:14" ); 
       lfma[ii]  = (complex double *) Malloc(lenu*sizeof(complex double), "pilu:15" ); 
       memcpy(lfma[ii], w, lenu*sizeof(complex double));
...
|--------------------------------------------------------------------*/
       lrowm = amat->U->ma[jrow];
       fact = w[jj] * lrowm[0];
       if ( cabs(fact) > drop3 ) {      /*  DROPPING IN E U^{-1}   */
	 lrowj = amat->U->ja[jrow];
	 lrowz = amat->U->nzcount[jrow];
	 rrowj = lfja[jrow];
	 rrowm = lfma[jrow];
...
|--------------------------------------------------------------------*/
     ...
     for (j=0; j<lenu; j++) {
       ...
       /* if (cabs(w2[j]) > tnorm) tnorm =  cabs(w2[j]); */
     if (tnorm == 0.0) {
       len = 1;
       w[0] = 1.0 + 0.0*I;  
       jw[0] = ii;
     ...
     if (jpos < len)
       zqsplitC(w, jw, len, jpos);
     schur->ma[ii] = (complex double *) Malloc(lenu*sizeof(complex double), "pilu:16" );
     schur->ja[ii] = (int *) Malloc(lenu*sizeof(int), "pilu:17" );
/*---------------------------------------------------------------------
|     copy ---
|--------------------------------------------------------------------*/
     memcpy(&schur->ja[ii][0], jw, jpos*sizeof(int));
     memcpy(&schur->ma[ii][0], w, jpos*sizeof(complex double));
   }
/*---------------------------------------------------------------------
