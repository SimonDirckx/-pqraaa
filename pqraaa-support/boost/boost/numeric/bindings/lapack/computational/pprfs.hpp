...
#else
    LAPACK_SPPRFS( &lapack_option< UpLo >::value, &n, &nrhs, ap, afp, b, &ldb,
            x, &ldx, ferr, berr, work, iwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CPPRFS( &lapack_option< UpLo >::value, &n, &nrhs, ap, afp, b, &ldb,
            x, &ldx, ferr, berr, work, rwork, &info ,1 );
#endif
    return info;
}
...
struct pprfs_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct pprfs_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
