...
inline std::ptrdiff_t heevr( const char jobz, const char range,
    ...
    LAPACK_CHEEVR( &jobz, &range, &lapack_option< UpLo >::value, &n, a, &lda,
            &vl, &vu, &il, &iu, &abstol, &m, w, z, &ldz, isuppz, work, &lwork,
            rwork, &lrwork, iwork, &liwork, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
struct heevr_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct heevr_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
