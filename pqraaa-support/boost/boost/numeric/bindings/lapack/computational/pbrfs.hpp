...
#else
    LAPACK_SPBRFS( &lapack_option< UpLo >::value, &n, &kd, &nrhs, ab, &ldab,
            afb, &ldafb, b, &ldb, x, &ldx, ferr, berr, work, iwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CPBRFS( &lapack_option< UpLo >::value, &n, &kd, &nrhs, ab, &ldab,
            afb, &ldafb, b, &ldb, x, &ldx, ferr, berr, work, rwork, &info ,1 );
#endif
    return info;
}
...
struct pbrfs_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const MatrixAB& ab, const MatrixAFB& afb,
        ...
        static_assert( (bindings::is_column_major< MatrixAFB >::value) );
        ...
        assert( bindings::stride_major(ab) >= bindings::bandwidth(ab,
                uplo())+1 );
        assert( bindings::stride_major(afb) >= bindings::bandwidth(ab,
                uplo())+1 );
...
//
template< typename Value >
struct pbrfs_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
