...
inline std::ptrdiff_t hegvx( const fortran_int_t itype, const char jobz,
    ...
    LAPACK_SSYGVX( &itype, &jobz, &range, &lapack_option< UpLo >::value, &n,
            a, &lda, b, &ldb, &vl, &vu, &il, &iu, &abstol, &m, w, z, &ldz,
            work, &lwork, iwork, ifail, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
inline std::ptrdiff_t hegvx( const fortran_int_t itype, const char jobz,
    ...
    LAPACK_CHEGVX( &itype, &jobz, &range, &lapack_option< UpLo >::value, &n,
            a, &lda, b, &ldb, &vl, &vu, &il, &iu, &abstol, &m, w, z, &ldz,
            work, &lwork, rwork, iwork, ifail, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
struct hegvx_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct hegvx_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
