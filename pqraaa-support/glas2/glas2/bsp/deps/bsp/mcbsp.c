...
void bsp_begin( const MCBSP_PROCESSOR_INDEX_DATATYPE P ) {
	...
	init->threads = malloc( ((size_t)P) * sizeof( pthread_t ) );
...
#ifdef MCBSP_USE_SPINLOCK
	init->condition     = malloc( ((size_t)P) * sizeof( unsigned char ) );
	init->mid_condition = malloc( ((size_t)P) * sizeof( unsigned char ) );
	...
	init->threadData = malloc( ((size_t)P) * sizeof( struct mcbsp_thread_data * ) );
...
MCBSP_PROCESSOR_INDEX_DATATYPE bsp_pid( void ) {
	const struct mcbsp_thread_data * const data = mcbsp_internal_const_prefunction();
	return (MCBSP_PROCESSOR_INDEX_DATATYPE)(data->bsp_id);
}
...
MCBSP_PROCESSOR_INDEX_DATATYPE bsp_nprocs( void ) {
	...
	if( data == NULL ) {
		...
		if( machine->num_reserved_cores >= machine->cores ) {
			...
			return (MCBSP_PROCESSOR_INDEX_DATATYPE)
				(machine->cores - machine->num_reserved_cores) *
				(machine->threads_per_core - machine->unused_threads_per_core);
		}
	} else {
		//check if the BSP execution was aborted
		mcbsp_internal_check_aborted();
		//return nprocs involved in this SPMD run
		return ((MCBSP_PROCESSOR_INDEX_DATATYPE)data->init->P);
	}
}
	...
	for( size_t s = 0; s < data->init->P; ++s ) {
		...
		while( !mcbsp_util_stack_empty( queue ) ) {
			...
			if( request->source == request->destination ) {
				//then communication here is useless, regardless of length; ignore this request
				continue;
			}
	...
	for( size_t s = 0; s < data->init->P; ++s ) {
		...
		while( !mcbsp_util_stack_empty( queue ) ) {
			...
			if( request->destination == NULL ) {
				...
				mcbsp_util_memcpy( request->destination, mcbsp_util_varstack_pop( queue, request->length ), request->length );
			}
		}
...
void bsp_put( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const source,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_put called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_get( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const source,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_get called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_direct_get( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const source,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_direct_get called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_send( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const tag,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_send called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_hpsend( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const tag,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_hpsend called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_hpput( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const source,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_hpput called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_hpget( const MCBSP_PROCESSOR_INDEX_DATATYPE pid, const void * const source,
	...
	//sanity check
	if( pid >= data->init->P ) {
		bsp_abort( "Error: bsp_hpget called with an out-of-range PID argument (%ld, while bsp_nprocs()==%ld)!\n",
			(unsigned long int)pid, (unsigned long int)(data->init->P) );
	}
#endif
...
void bsp_get_tag( MCBSP_BYTESIZE_TYPE * const status, void * const tag ) {
	...
	if( mcbsp_util_stack_empty( &(data->bsmp) ) ) {
		...
		*status = (MCBSP_BYTESIZE_TYPE) size;
...
MCBSP_BYTESIZE_TYPE bsp_hpmove( const void* * const p_tag, const void* * const p_payload ) {
	//get thread data
	struct mcbsp_thread_data * const data = mcbsp_internal_prefunction();
