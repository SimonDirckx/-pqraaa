...
namespace boost { namespace numeric { namespace bindings { namespace mumps {
  ...
  namespace detail {
    ...
    //
    template <typename M>
    void indices( bindings::tag::row_major, MUMPS_INT*& rows, MUMPS_INT*& cols, M const& m ) {
      rows = const_cast<MUMPS_INT*>( bindings::begin_index_major( m ) ) ;
      cols = const_cast<MUMPS_INT*>( bindings::begin_index_minor( m ) ) ;
    }
  
    template <typename M>
    void indices( bindings::tag::column_major, MUMPS_INT*& rows, MUMPS_INT*& cols, M const& m ) {
      cols = const_cast<MUMPS_INT*>( bindings::begin_index_major( m ) ) ;
      rows = const_cast<MUMPS_INT*>( bindings::begin_index_minor( m ) ) ;
    }
  ...
  void matrix_integer_data( mumps<M>& data, M& m ) {
    ...
    static_assert( index_b::value == 1 ) ;
    data.n = bindings::num_rows( m ) ;
    assert( bindings::num_columns( m ) == data.n ) ;
  ...
  void rhs_sol_value_data( mumps<M>& data, X& x ) {
    ...
    data.nrhs = bindings::size_major( x ) ;
