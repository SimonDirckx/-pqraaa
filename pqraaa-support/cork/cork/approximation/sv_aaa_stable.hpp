...
namespace CORK { namespace approximation {
  ...
  typename std::enable_if< glas2::is<glas2::DenseMatrix, FunctionValues>::value
    ...
    SV_AAA_approximation<value_type> repr_best( test_values.num_columns(), options::value_of<options::max_degree>(options) ) ;
    repr_best.reset( 0 ) ;
    ...
    for (int d=0; d<options::value_of<options::max_degree>(options) && repr.error()>options::value_of<options::rational_approximation_tolerance<real_type>>(options); ++d) {
