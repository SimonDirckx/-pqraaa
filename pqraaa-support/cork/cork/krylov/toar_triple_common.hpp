...
#ifndef cork_krylov_toar_triple_common_hpp
#define cork_krylov_toar_triple_common_hpp
...
namespace CORK { namespace krylov {
  ...
  class toar_triple_common {
    public:
      typedef T                                                    value_type ;
      typedef glas2::shared_matrix< value_type >                          sharedmatrix_type ;
      typedef glas2::matrix< value_type >                          matrix_type ;
      typedef typename matrix_type::size_type                      size_type ;
      typedef typename std::decay<Options>::type                   options_type ;
      
    private:
      typedef decltype(std::abs(value_type()))                     real_value_type ;

    public:
      toar_triple_common( size_type n, size_type k_max, size_type degree, options_type const& options )
      : Q( n, k_max+degree )
    ...
    public:
      ...
      : toar_triple( linearization.size()
                   , (CORK::options::value_of<options::max_krylov_dimension>(options)==0? 2*n_wanted : CORK::options::value_of<options::max_krylov_dimension>(options))
      ...
      bool initial_vector( size_type new_rank, bool q_already_orto=true ) {
        ...
        if (!q_already_orto) {
          for (size_type i=rank; i<new_rank; ++i) {
            add_column_of_Q( 0 ) ;
            inner_prod_.add_column_of_Q( Q, i ) ;
          }
        } else {
          for (size_type i=rank; i<new_rank; ++i) {
            inner_prod_.add_column_of_Q( Q, i ) ;
        ...
        if (nU!=0) {
        std::cout << "U(glas2::all(),0) = " << U(glas2::all(),0) << std::endl ;
          U( glas2::all(), 0 ) /= nU ;
        std::cout << "U(glas2::all(),0) = " << U(glas2::all(),0) << std::endl ;
        } else {
          return false ;
    ...
    private:
      ...
      void add_column_of_Q( size_type column ) {
        ...
        if (norm>CORK::options::value_of<options::q_drop_tol>(options_)*norm_q) {
      ...
      void gram_schmidt_u( size_type column ) {
        ...
        std::cout << "U(glas2::all(),r_step) = " << U(glas2::all(),r_step) << std::endl ;
        std::cout << "U(glas2::all(),column) = " << U(glas2::all(),column) << std::endl ;
        inner_prod_( conj(U(glas2::all(),r_step)), U(glas2::all(),column), g );
        ...
        H( column, column-1 ) = inner_prod_.norm( u_vector(column)(glas2::all(),glas2::range(0,rank)) ) ;
        std::cout << "H( column, column-1 ) = " << H( column, column-1 ) << std::endl ;
    ...
    public:
      ...
      void transform_vectors( PP const& P ) {
        ...
        if (value_of<options::debug_level>(options_)>3) std::cout << "SVD of truncation of U " << svd_val << std::endl ;
        for (size_type i=1; i<new_rank; ++i) {
          if (svd_val(i)<value_of<options::Q_drop_tol>(options_)*svd_val(1)) {
            new_rank = i ;
            break ;
          }
        }
        if (value_of<options::debug_level>(options_)>2) std::cout << "rank change after basis transformation from " << rank << " to " << new_rank << std::endl ;
