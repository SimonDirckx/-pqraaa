...
int main() {
    ...
    std::cout << "auto t1_p = glas3::permute( t1, {1, 2, 0} ) ;" << '\n' << '\n' ;
    auto t1_p = glas3::permute( t1, {1, 2, 0} ) ;
    glas3::randomize( t1, g ) ;
    auto t1_full_p = glas3::permute( glas3::full( t1 ), {1, 2, 0} );

    std::cout << "glas3::full( t1_p ): " << '\n' << glas3::full( t1_p ) << '\n' ;
    std::cout << "t1_full_p: " << '\n' << t1_full_p << '\n' ;
    ...
    auto t2_p = glas3::permute( t2, {2, 0, 1} ) ;
    glas3::randomize( t2, g ) ;
    auto t2_full_p = glas3::permute( glas3::full( t2 ), {2, 0, 1} );
