...
#else
    LAPACK_STREVC( &lapack_option< Side >::value, &howmny, select, &n, t,
            &ldt, vl, &ldvl, vr, &ldvr, &mm, &m, work, &info ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CTREVC( &lapack_option< Side >::value, &howmny, select, &n, t,
            &ldt, vl, &ldvl, vr, &ldvr, &mm, &m, work, rwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct trevc_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct trevc_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
