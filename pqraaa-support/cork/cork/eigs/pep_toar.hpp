...
#ifndef cork_eigs_pep_toar_hpp
#define cork_eigs_pep_toar_hpp
...
#include <cork/linearization/make_cork_linearization.hpp>
...
namespace CORK { namespace eigs { namespace AAA_TOAR {
  ...
  decltype (auto) compute_eigenvalues_and_vectors( NEP const& problem, Domain const& domain, int n_wanted, Options const& options ) {
    ...
    CORK::krylov::toar_triple<value_type,decltype(options.krylov_options)> triple( linearization, eigenvalue_selector, options.krylov_options ) ;
