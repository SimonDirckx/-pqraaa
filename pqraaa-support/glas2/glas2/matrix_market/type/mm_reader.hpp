...
namespace glas2 { namespace matrix_market {
  ...
  class mm_reader {
    ...
    public:
      struct iterator {
        iterator( std::istream& s, size_type index, field_type f, rep_type rep, symmetry_type symm, size_type num_rows )
        : s_( s )
        , index_( index )
        , field_( f )
        , rep_( rep )
        ...
        void read() {
          ...
          if (rep_==COORDINATE) {
            ...
            ++row_ ;
            if (row_==num_rows_) { row_=0; ++column_ ; }
          }
          if (field_==REAL) {
      ...
      iterator begin() const { return iterator( s_, 0, field(), rep(), symmetry(), num_rows_ ) ; }
      iterator end() const { return iterator( s_, num_nz_, field(), rep(), symmetry(), num_rows_ ) ; }
    ...
    private:
      inline void read() {
        ...
        if ( str!="%%MatrixMarket" ) {
           throw std::runtime_error(std::string( "Not a Matrix Market file: I got on first line: ") + str) ;
        }

        std::getline( s_, str, ' ' ) ;
        if ( str!="matrix" && str!="mtx" ) {
           throw std::runtime_error(std::string( "Not a Matrix Market file for a matrix" )) ;
        }
        ...
        else throw std::runtime_error(std::string( "Invalid rep" )) ;
...
//        else if (str=="hermitian") symmetry_ = HERMITIAN ;
        else throw std::runtime_error(std::string( "Invalid symmetry for GLAS2" )) ;
...
namespace glas2 {
  ...
  template <typename T>
  struct glas_concept< matrix_market::mm_reader<T> >
