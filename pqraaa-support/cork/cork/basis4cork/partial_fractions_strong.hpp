...
#ifndef cork_basis4cork_partial_fractions_strong_hpp
#define cork_basis4cork_partial_fractions_strong_hpp

#include <cork/basis/partial_fractions_strong.hpp>
#include <cork/basis4cork/partial_fractions_strong_handle.hpp>
...
namespace CORK { namespace basis4CORK {
  ...
  class basis4CORK< basis::partial_fractions_strong<Poles,Weights,I> >
  {
    public:
    ...
    public:
      explicit basis4CORK( basis::partial_fractions_strong<Poles,Weights,I> const& basis )
      : poles_( basis.poles() )
    ...
    public:
      ...
      void multiply_N( UM const& U, ZM Z ) const {
        ...
        for (int i=1; i<size(); ++i)
          Z(i-1,glas2::all()) = -weights_(i) * U( i-1, glas2::all() ) + weights_(i-1) * U( i, glas2::all() ) ;
      } // multiply_N()
      ...
      void multiply_M( UM const& U, ZM Z ) const {
        ...
        for (int i=1; i<size(); ++i)
          Z(i-1,glas2::all()) = weights_(i-1) * poles_(i-1) * U( i, glas2::all() ) - weights_(i) * poles_(i) * U( i-1, glas2::all() ) ;
      } // multiply_M()

    public:
      template <typename ValueType>
      using handle_type = partial_fractions_strong_handle< ValueType, poles_type const&, weights_type const&, size_type > ;
