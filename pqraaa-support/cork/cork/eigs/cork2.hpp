...
namespace CORK { namespace eigs {
 ...
 info cork( Linearization& linearization, EigSelector const& eigenvalue_selector, ShiftGenerator & shift_generator, InitialVector const& initial_vector, Quadruple& quadruple,
   ...
   //process.initialize( shift_generator.shift(0) ) ; /// NOT NEEDED
   ...
   while ( !converged && information.number_of_solves+quadruple.k_max() < options::value_of<options::max_solves>(options)+quadruple.k() ) {
     if (options::value_of<options::debug_level>(options)>0) std::cout << "---------- New sweep of iterations or restart --------" << std::endl ;
     ...
     // Schur decomposition
     //
     //     [H_L  *   * ]   [ I_L  0   0  ]   [ I_L  0   0  ]   [ T_L T_L,P  * ]
     //     [ 0  H_P  * ] . [  0  I_P  0  ] = [  0  I_P  0  ] . [  0   T_P   *  ]
     //     [ 0   0  H_U]   [  0   0  X_U ]   [  0   0  Y_U ]   [  0    0   T_U ]
     //
     //     [K_L  *   * ]   [ I_L  0   0  ]   [ I_L  0   0  ]   [ S_L S_L,P  * ]
     //     [ 0  K_P  * ] . [  0  I_P  0  ] = [  0  I_P  0  ] . [  0   S_P   *  ]
     //     [ 0   0  K_U]   [  0   0  X_U ]   [  0   0  Y_U ]   [  0    0   S_U ]
     //
     ...
     glas2::range range_u(n_locked,quadruple.k()) ;
...
#ifndef NDEBUG
     // Allow to check the Schur decompositions and reorderings in debug mode.
     ...
     {
       auto schur_matrix_H_u = quadruple.H( range_u, range_u ) ;
       auto schur_matrix_K_u = quadruple.K( range_u, range_u ) ;
       auto schur_vec_X_u = schur_vectors_X( range_u, range_u ) ;
       auto schur_vec_Y_u = schur_vectors_Y( range_u, range_u ) ;
       auto lambda_val_u = lambda_values( range_u ) ;
       lapack::schur( schur_matrix_K_u, schur_matrix_H_u, schur_vec_X_u, schur_vec_Y_u, lambda_val_u ) ;
     }
     ...
     // Sort locked part first:
     // split L and P.
     int keep_unlocked = 0 ;
     int old_n_locked = n_locked ;
     glas2::vector<int> order_unlocked( order.size() ) ;
     {
       ...
       if (ierr==1 && schur_size<keep_locked) {
         information.warnings.push_back( "Schur ordering of locked part failed: all eigenvalues are retained" ) ;
       }
       for (int i=0; i<num_keep; ++i) {
         if (n_locked<=order(i)) {
       ...
       n_locked = schur_size ;
       assert( num_keep==keep_unlocked+keep_locked ) ;
       order_unlocked( glas2::range(0,keep_unlocked) ) -= n_locked ;
     }
     assert (norm_fro( schur_vectors_X( range_l, range_u ) )==0.0 ) ;
     assert (norm_fro( schur_vectors_Y( range_l, range_u ) )==0.0 ) ;

     // Update the (1,2) part of the P part.
     {
       glas2::range range_l_nl( /*n_locked*/0, old_n_locked ) ;
       auto schur_vec_X_u = schur_vectors_X( range_u, range_u ) ;
       auto schur_vec_Y_l = schur_vectors_Y( range_l, range_l_nl ) ;
       glas2::matrix<value_type> temp( range_l.size(), range_u.size() ) ;

       temp = multiply( quadruple.H( range_l, range_u ), schur_vec_X_u ) ;
       quadruple.H( range_l_nl, range_u ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;

       temp = multiply( quadruple.K( range_l, range_u ), schur_vec_X_u ) ;
       quadruple.K( range_l_nl, range_u ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;
     }

     // range_l refers to locked part L
     range_l = glas2::range ( 0, n_locked ) ;
     // range_u refers tot the rest: P & U.
     range_u = glas2::range ( n_locked, quadruple.k() ) ;

     // Reorder the unlocked part
     if (keep_unlocked>0) {
     //  assert (norm_fro( schur_vectors_X( range_l, range_u ) )==0.0 ) ;
     //  assert (norm_fro( schur_vectors_Y( range_l, range_u ) )==0.0 ) ;
       ...
       auto schur_matrix_H_u = quadruple.H( range_u, range_u ) ;
       auto schur_matrix_K_u = quadruple.K( range_u, range_u ) ;
       //auto schur_vec_X = schur_vectors_X( range_u, range_u ) ;
       //auto schur_vec_Y = schur_vectors_Y( range_u, range_u ) ;
       glas2::matrix<value_type> permute_X( range_u.size(), range_u.size() ) ;
       glas2::matrix<value_type> permute_Y( range_u.size(), range_u.size() ) ;
       permute_Y = permute_X = glas2::identity_matrix<value_type>( permute_X.num_rows(), permute_X.num_columns() ) ;
       auto lambda_val = lambda_values( range_u ) ;
       int schur_size ;
       int ierr = lapack::order_schur( schur_matrix_K_u, schur_matrix_H_u, permute_X, permute_Y, lambda_val, order_unlocked( glas2::range(0,keep_unlocked) ), schur_size ) ;
       if (ierr==1 && schur_size<keep_unlocked) {
         information.warnings.push_back( "Schur ordering of unlocked part failed: less eigenvalues are retained" ) ;
         keep_unlocked = schur_size ;
       }
       if (schur_size==0) {
          information.error = "Failure in reordering of the Schur form of unlocked part using LAPACK: *TGSEN" ;
          information.number_converged_and_wanted = 0 ;
          information.number_converged = 0 ;
          return information ;
       }
       glas2::range range_keep_l( n_locked, keep_unlocked+n_locked ) ;
       glas2::matrix<value_type> temp( schur_vec_X.num_rows(), keep_unlocked ) ;
       temp = multiply( schur_vec_X(glas2::all(), range_u), permute_X( glas2::all(), glas2::range( 0, keep_unlocked ) ) ) ;
       schur_vec_X( glas2::all(), range_keep_l ) = temp ;
       temp = multiply( schur_vec_Y(glas2::all(), range_u), permute_Y( glas2::all(), glas2::range( 0, keep_unlocked ) ) ) ;
       schur_vec_Y( glas2::all(), range_keep_l ) = temp ;

       auto temp2 = temp( range_l, glas2::all() ) ;
       temp2 = multiply( quadruple.H( range_l, range_u ), permute_X( glas2::all(), glas2::range( 0, keep_unlocked ) ) ) ; 
       quadruple.H( range_l, range_keep_l ) = temp2 ;
       temp2 = multiply( quadruple.K( range_l, range_u ), permute_X( glas2::all(), glas2::range( 0, keep_unlocked ) ) ) ; 
       quadruple.K( range_l, range_keep_l ) = temp2 ;
     }
     num_keep = n_locked + keep_unlocked ;
     sort_ritz_values.keep( num_keep ) ;
     glas2::range range_keep( 0, num_keep ) ;

     glas2::range range_keep_l( n_locked, num_keep ) ;
     // Update the (1,2) part of the Schur matrices after the reordering of the unlocked part
     /*{
       glas2::range range_o_l( 0, old_n_locked ) ;
       glas2::range range_o_lu( n_locked, old_n_locked ) ;
       auto schur_vec_X_u = schur_vectors_X( range_u, range_keep_l ) ;
       auto schur_vec_Y_l = schur_vectors_Y( range_k, range_u ) ;
       glas2::matrix<value_type> temp( range_o_l.size(), range_keep_l.size() ) ;

       temp = multiply( quadruple.H( range_l, range_u ), schur_vec_X_u ) ;
       quadruple.H( range_l, range_keep_l ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;

       temp = multiply( quadruple.K( range_o_l, range_u ), schur_vec_X_u ) ;
       quadruple.K( range_l, range_keep_l ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;
     }*/
/*     glas2::matrix<value_type> temp_h( H_orig.num_rows(), range_keep.size() ) ;
     temp_h = multiply(H_orig, schur_vec_X(range_k,range_keep) ) ;
     quadruple.H( range_keep, range_keep ) = multiply( conj(transpose(schur_vec_Y(range_k, range_keep))), temp_h ) ;
     temp_h = multiply(K_orig, schur_vec_X(range_k,range_keep) ) ;
     quadruple.K( range_keep, range_keep ) = multiply( conj(transpose(schur_vec_Y(range_k, range_keep))), temp_h ) ;
*/
     ...
     if (options::value_of<options::debug_level>(options)>1) {
       std::cout << "Error on X after restart " << norm_fro( multiply( conj(transpose(schur_vec_X(glas2::all(), range_keep))), schur_vec_X(glas2::all(), range_keep) )-glas2::identity_matrix<value_type>(range_keep.size(),range_keep.size())) << std::endl ;
       std::cout << "Error on Y after restart " << norm_fro( multiply( conj(transpose(schur_vec_Y(glas2::all(), range_keep))), schur_vec_Y(glas2::all(), range_keep) )-glas2::identity_matrix<value_type>(range_keep.size(),range_keep.size())) << std::endl ;
       std::cout << "Error on H after restart " << norm_fro( multiply( H_orig, schur_vec_X(range_k, range_keep)) - multiply( schur_vec_Y(range_k, range_keep), quadruple.H( range_keep, range_keep) ) ) << std::endl ;
       std::cout << "Error on K after restart " << norm_fro( multiply( K_orig, schur_vec_X(range_k, range_keep)) - multiply( schur_vec_Y(range_k, range_keep), quadruple.K( range_keep, range_keep) ) ) << std::endl ;
     }
#endif
     ...
     if (information.number_converged<num_keep) {
       int schur_size ;
       ...
       if (ierr==1) {
          information.error = "Failure in final reordering of the Schur form using LAPACK: *TGSEN" ;
          num_keep = schur_size ;
