...
#else
    LAPACK_STRCON( &norm, &lapack_option< UpLo >::value, &lapack_option<
            Diag >::value, &n, a, &lda, &rcond, work, iwork, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CTRCON( &norm, &lapack_option< UpLo >::value, &lapack_option<
            Diag >::value, &n, a, &lda, &rcond, work, rwork, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
struct trcon_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct trcon_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
