...
#ifndef glas3_array_dense_array_type_shape_index_hpp
#define glas3_array_dense_array_type_shape_index_hpp
...
public:
	...
	shape_index ( index_type shape_out )
...
public:
	...
	shape_index ( shape_index const& that )
	...
	// Move constructor
	shape_index ( shape_index && that ) = default ;

public:
	// Copy assignment -> do not allow
	shape_index& operator= ( shape_index const& that ) = delete ; // not assignable

	// Move assignment
	shape_index& operator= ( shape_index&& that) = delete ;

private:
	// Constructor used in shallow_copy
	shape_index ( boost::shared_ptr<index_type> shape_out, boost::shared_ptr<index_type> dim_factor_out,
			boost::shared_ptr<index_type> j_out, value_type lin_size, value_type overflow_count,
			boost::shared_ptr<value_type> i_out, boost::shared_ptr<shape_type> shape, size_type size)
...
public:
	shape_index shallow_copy () const {
		return shape_index( shape_out_, dim_factor_out_, j_out_, lin_size_, overflow_count_, i_out_, shape_, size_ );
	}
...
template <typename V>
struct concept< shape_index<V>, typename std::enable_if< std::is_integral<V>::value >::type >
: Index
  {} ;
