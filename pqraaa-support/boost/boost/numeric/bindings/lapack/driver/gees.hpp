...
#else
    LAPACK_SGEES( &jobvs, &sort, select, &n, a, &lda, &sdim, wr, wi, vs,
            &ldvs, work, &lwork, bwork, &info ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CGEES( &jobvs, &sort, select, &n, a, &lda, &sdim, w, vs, &ldvs,
            work, &lwork, rwork, bwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct gees_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct gees_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
