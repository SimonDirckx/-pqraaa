...
#ifndef cork_approximation_sv_rat_real_least_squares_hpp
#define cork_approximation_sv_rat_real_least_squares_hpp
...
namespace CORK { namespace approximation {
  ...
  auto SV_rat_real_least_squares( SamplePoints const& test_set, FunctionValues const& test_values, Poles const& poles, int degree, Options const& options ) {
    ...
    CORK::basis::monomial basis1(degree) ;
    CORK::basis::partial_fractions_real basis2(poles_c,weights) ;
    ...
    return set_valued_function< value_type, decltype(basis) >( basis, rhs( glas2::range(0,poles_c.size()+degree+1), glas2::all() ) ) ;
  } // SV_rat_real_least_squares()


  struct SV_rat_real_least_squares_selector {
    template <typename SamplePoints, typename FunctionValues, typename Poles, typename Options>
    auto operator() ( SamplePoints const& test_set, FunctionValues const& test_values, Poles const& poles, int degree, Options const& options ) const {
      return SV_rat_real_least_squares( test_set, test_values, poles, degree, options ) ;
    }
  } ; // struct SV_rat_real_least_squares_selector
