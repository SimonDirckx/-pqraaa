...
#else
    LAPACK_ZCPOSV( &lapack_option< UpLo >::value, &n, &nrhs, a, &lda, b, &ldb,
            x, &ldx, work, swork, rwork, &iter, &info ,1 );
#endif
    return info;
}
...
struct iter_posv_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct iter_posv_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
//
template< typename MatrixA, typename MatrixB, typename MatrixX >
inline typename std::enable_if<! detail::is_workspace< MatrixX >::value,
        std::ptrdiff_t >::type
