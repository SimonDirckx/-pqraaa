...
#else
    LAPACK_SSPGVD( &itype, &jobz, &lapack_option< UpLo >::value, &n, ap, bp,
            w, z, &ldz, work, &lwork, iwork, &liwork, &info ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CHPGVD( &itype, &jobz, &lapack_option< UpLo >::value, &n, ap, bp,
            w, z, &ldz, work, &lwork, rwork, &lrwork, iwork, &liwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct hpgvd_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct hpgvd_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
