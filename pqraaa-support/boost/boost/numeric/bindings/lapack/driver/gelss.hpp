...
struct gelss_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct gelss_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
//
template< typename MatrixA, typename MatrixB, typename VectorS >
inline typename std::enable_if<! detail::is_workspace< VectorS >::value,
        std::ptrdiff_t >::type
gelss( MatrixA& a, MatrixB& b, VectorS& s,
        const typename remove_imaginary< typename bindings::value_type<
        MatrixA >::type >::type rcond, fortran_int_t& rank ) {
