...
struct gtrfs_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const fortran_int_t n,
        ...
        assert( bindings::size(df) >= n );
        assert( bindings::size(dl) >= n-1 );
        assert( bindings::size(dlf) >= n-1 );
        assert( bindings::size(du) >= n-1 );
        assert( bindings::size(du2) >= n-2 );
        assert( bindings::size(duf) >= n-1 );
        assert( bindings::size(ipiv) >= n );
...
//
template< typename Value >
struct gtrfs_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
