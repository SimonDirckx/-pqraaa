...
#endif
				case 19:{ return new CCSWrapper< double, vecBICRS< double >, ULI >( input, m, n, zero ); }
#ifdef WITH_CUDA
...
#endif
				case 19:{ return new vecBICRS< double >( input, m, n, zero ); }
#ifdef WITH_CUDA
...
#endif
				case 19:{ return new CCSWrapper< double, vecBICRS< double >, ULI >( file, zero ); }
#ifdef WITH_CUDA
...
#endif
				case 19:{ return new vecBICRS< double >( file, zero ); }
#ifdef WITH_CUDA
...
int main( int argc, char** argv ) {
	...
	if( argc<=3 ) {
		...
		std::cout << "12: Beta Hilbert (known as Block CO-H+ in the paper by Yzelman & Roose, 2014: parallel compressed blocked Hilbert with BICRS)" << std::endl;
		std::cout << "13: Row-distributed Beta Hilbert (known as Row-distributed block CO-H in the paper by Yzelman & Roose, 2014: same as 12, but simpler distribution)" << std::endl;
#ifdef WITH_CSB
		...
		std::cout << "16: Row-distributed parallel CRS (using OpenMP, known as OpenMP CRS in the paper by Yzelman & Roose, 2014)" << std::endl;
...
#endif
		std::cout << "19: Vectorised BICRS." << std::endl;
#ifdef WITH_CUDA
		...
		std::cout << "\n\nReferences:\n";
		std::cout << "===========\n\n";
		std::cout << "Schemes 3, 4 are published in:\n";
		std::cout << "1. Cache-oblivious sparse matrix-vector multiplication by using sparse matrix partitioning methods by A. N. Yzelman & Rob H. Bisseling\n";
		std::cout << "   in SIAM Journal of Scientific Computation 31(4), pp. 3128-3154 (2009).\n\n";
		std::cout << "Schemes 6, 7, 8 are published in:\n";
		std::cout << "1. A cache-oblivious sparse matrix-vector multiplication scheme based on the Hilbert curve by A. N. Yzelman & Rob H. Bisseling\n";
		std::cout << "   in M. Günther, A. Bartel, M. Brunk, S. Schöps, and M. Striebel (eds.), Progress in Industrial Mathematics at ECMI 2010, pp. 627-634, Springer, 2012.\n";
		std::cout << "2. A Hilbert-order multiplication scheme for unstructured sparse matrices by G. Haase, M.  Liebmann, and G. Plank\n";
		std::cout << "   in International Journal of Parallel, Emergent and Distributed Systems 22(4), pp.213-220 (2007).\n\n";
		std::cout << "Schemes 11, 12, 13, 16 are published in:\n";
		std::cout << "1. High-Level Strategies for Parallel Shared-Memory Sparse Matrix–Vector Multiplication by A. N. Yzelman & D. Roose\n";
		std::cout << "   in IEEE Transactions on Parallel and Distributed Systems 25(1), pp. 116-125 (2014).\n\n";
		std::cout << "Scheme 19 and the `micbench' utility are published in:\n";
		std::cout << "1. Sparse matrix-vector multiplication: parallelization and vectorization by A. N. Yzelman, D. Roose, & K. Meerbergen\n";
		std::cout << "   in J. Reinders and J. Jeffers (eds.), High Performance Parallelism Pearls, Chapter 27, ISBN 978-01-280-2118-7, Elsevier, 2014.\n";
		std::cout << "2. Sparse matrix computations on multi-core systems by A. N. Yzelman & D. Roose\n";
		std::cout << "   Intel European Exascale Labs report 2013, pp. 24-29, Intel, 2014.\n\n";
		std::cout << "Other schemes (McDMV, McShared, McDirect, sbdmv, sbd2trp utilities) are published in:\n";
		std::cout << "1. Two-dimensional cache-oblivious sparse matrix-vector multiplication by A. N. Yzelman & Rob H. Bisseling\n";
		std::cout << "   in Parallel Computing 37(12), pp. 806-819 (2011).\n";
		std::cout << "2. Fast sparse matrix-vector multiplication by partitioning and reordering by A. N. Yzelman\n";
		std::cout << "   Ph.D. Dissertation, Utrecht University, ISBN 978-90-393-5590-9, 2011.\n\n";
		return EXIT_SUCCESS;
	}
...
#endif
	{
		if( posix_memalign( (void**)&x, 64, matrix->n() * sizeof( double ) ) != 0 ) {
			std::cerr << "Could not allocate input vector!" << std::endl;
			exit( EXIT_FAILURE );
		}
	}
...
#ifdef _NO_LIBNUMA
	if( scheme == 12 || scheme == 13 ) {
		_mm_free( z );
	} else if( scheme == 14 || scheme == 15 || scheme == 16 || scheme == 17 || scheme == 18 ) {
		free( z );
	}
#else
