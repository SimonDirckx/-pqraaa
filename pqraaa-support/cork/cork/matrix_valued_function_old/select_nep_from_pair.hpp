...
namespace CORK { namespace matrix_valued_function {
  ...
  typename std::enable_if< matrix_polynomial<Basis,CoefficientMatrices>::template has_value_type_for<T>::value
                         , matrix_polynomial<Basis,CoefficientMatrices> const&
                         >:: type select_nep_from_pair( matrix_polynomial<Basis,CoefficientMatrices> const& nep ) {
    return nep ;
  }
...
*/
  ...
  auto const& select_nep_from_pair( std::pair<NEP1,NEP2> const& pair ) {
    return select_nep_from_pair_traits<T,AllowFail,NEP1,NEP2>::apply( pair.first, pair.second ) ;
  }
