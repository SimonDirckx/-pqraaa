 ...
 info cork( Linearization& linearization, EigSelector const& eigenvalue_selector, ShiftGenerator & shift_generator, InitialVector const& initial_vector, Quadruple& quadruple,
   ...
   /*  if (n_locked>0) {
     ...
     // Update the (1,2) part of the P part.
     {
       ...
       auto schur_vec_X_k_l = schur_vectors_X( range_k_l, range_o_k_l ) ;
       ...
       quadruple.H( range_o_l, range_o_k_l ) = multiply( conj(transpose(schur_vec_Y_l)), temp ) ;

       temp = multiply( quadruple.K( range_o_l, range_k_l ), schur_vec_X_k_l ) ;
