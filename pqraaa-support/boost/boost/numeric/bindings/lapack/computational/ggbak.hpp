...
#else
    LAPACK_SGGBAK( &job, &lapack_option< Side >::value, &n, &ilo, &ihi,
            lscale, rscale, &m, v, &ldv, &info ,1 ,1 );
#endif
    return info;
}
...
struct ggbak_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const char job, const Side side,
        ...
        assert( bindings::size(lscale) >= bindings::size_row(v) );
        assert( bindings::size(rscale) >= bindings::size_row(v) );
...
//
template< typename Value >
struct ggbak_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
