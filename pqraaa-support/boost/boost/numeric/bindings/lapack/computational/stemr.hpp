...
#else
    LAPACK_SSTEMR( &jobz, &range, &n, d, e, &vl, &vu, &il, &iu, &m, w, z,
            &ldz, &nzc, isuppz, &tryrac, work, &lwork, iwork, &liwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct stemr_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct stemr_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
