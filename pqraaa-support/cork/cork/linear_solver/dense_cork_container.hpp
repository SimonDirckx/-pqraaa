...
#ifndef cork_linear_solver_dense_cork_container_hpp
#define cork_linear_solver_dense_cork_container_hpp
...
#include <cork/exception/linear_solver_prepare_failure.hpp>
...
namespace CORK { namespace linear_solver {
  ...
  class dense_cork_container  
  {
    ...
    public:
      ...
      void prepare_solve( Accumulate const& accumulate ) {
        ...
        assert( info>=0 ) ;
        if (info>0) throw exception::linear_solver_prepare_failure() ;
      } // prepare_solve()
  ...
  struct linear_solver_traits< T, coefficient_matrices::dense<Matrices> > {
    typedef dense_cork_container< typename std::common_type<T, typename coefficient_matrices::dense_cork_container<Matrices>::value_type>::type > type ;

    static type apply( coefficient_matrices::dense_cork_container<Matrices> const& d ) {
  ...
  template <typename T, typename Matrices>
  struct linear_solver_traits< T, coefficient_matrices::any_glas<Matrices>, typename std::enable_if< glas2::is<glas2::DenseMatrix, typename std::decay<Matrices>::type::value_type>::value >::type > {
    typedef dense_cork_container< typename std::common_type<T, typename coefficient_matrices::any_glas<Matrices>::value_type>::type > type ;
