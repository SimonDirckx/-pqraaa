...
//
// The LAPACK-backend for syrfs is the netlib-compatible backend.
//
...
#else
    LAPACK_SSYRFS( &lapack_option< UpLo >::value, &n, &nrhs, a, &lda, af,
            &ldaf, ipiv, b, &ldb, x, &ldx, ferr, berr, work, iwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CSYRFS( &lapack_option< UpLo >::value, &n, &nrhs, a, &lda, af,
            &ldaf, ipiv, b, &ldb, x, &ldx, ferr, berr, work, rwork, &info ,1 );
#endif
    return info;
}
...
struct syrfs_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const MatrixA& a, const MatrixAF& af,
        ...
        static_assert( (bindings::is_column_major< MatrixAF >::value) );
        ...
        assert( bindings::size(ipiv) >= bindings::size_column(a) );
...
//
template< typename Value >
struct syrfs_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
// Calls to these functions are passed to the syrfs_impl classes. In the 
