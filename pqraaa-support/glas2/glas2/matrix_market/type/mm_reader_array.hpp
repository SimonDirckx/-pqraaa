...
#ifndef glas2_matrix_market_type_mm_reader_array_hpp
#define glas2_matrix_market_type_mm_reader_array_hpp

#include <glas2/sparse/concept/forward_dense_vector.hpp>
...
namespace glas2 { namespace matrix_market {
  ...
  template <typename T>
  class mm_reader_array {
    public:
      typedef T   value_type ;
      typedef int size_type ;

    public:
      mm_reader_array( std::istream& is )
      : s_ (is )
      {
        read() ;
      }
...
namespace glas2 {
  ...
  template <typename T>
  struct concept< matrix_market::mm_reader_array<T> >
  : ForwardDensevector
  {} ;
