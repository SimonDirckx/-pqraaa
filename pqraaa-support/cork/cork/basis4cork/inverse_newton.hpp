...
#ifndef cork_basis4cork_inverse_newton_hpp
#define cork_basis4cork_inverse_newton_hpp

#include <cork/basis/inverse_newton.hpp>
#include <cork/basis4cork/inverse_newton_handle.hpp>
...
namespace CORK { namespace basis4CORK {
  ...
  template <typename Points, typename Weighsts, typename I>
  class basis4CORK< basis::inverse_newton<Points,I> >
  {
    public:
      typedef typename std::decay<Points>::type  points_type ;
      typedef typename std::decay<Weighst>::type weights_type ;
      typedef I                                  size_type ;

      template< typename T>
      using handle_type = inverse_newton_handle<T, points_type const&, weights_type const&, size_type> ;

      template< typename T>
      using value_type = typename handle_type<T>::value_type ;

    public:
      explicit basis4CORK( basis::inverse_newton<Points,I> const& basis )
    ...
    public:
      ...
      handle_type<T> handle() const {
        return handle_type<T>( points_, weights_ ) ;
      }
