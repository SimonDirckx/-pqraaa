...
#else
    LAPACK_SGECON( &norm, &n, a, &lda, &anorm, &rcond, work, iwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_DGECON( &norm, &n, a, &lda, &anorm, &rcond, work, iwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CGECON( &norm, &n, a, &lda, &anorm, &rcond, work, rwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_ZGECON( &norm, &n, a, &lda, &anorm, &rcond, work, rwork, &info ,1 );
#endif
    return info;
}
...
struct gecon_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct gecon_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
//
template< typename MatrixA >
inline typename std::enable_if<! detail::is_workspace< MatrixA >::value,
        std::ptrdiff_t >::type
gecon( const char norm, const MatrixA& a,
        const typename remove_imaginary< typename bindings::value_type<
        MatrixA >::type >::type anorm, typename remove_imaginary<
