...
#else
    LAPACK_SSPSVX( &fact, &lapack_option< UpLo >::value, &n, &nrhs, ap, afp,
            ipiv, b, &ldb, x, &ldx, &rcond, ferr, berr, work, iwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct spsvx_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const char fact, const MatrixAP& ap,
        ...
        static_assert( (bindings::is_mutable< MatrixAFP >::value) );
...
//
template< typename Value >
struct spsvx_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
