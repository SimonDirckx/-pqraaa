...
struct largv_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const fortran_int_t n, VectorX& x,
        ...
        assert( bindings::size(c) >= 1+(n-1)*bindings::stride(c) );
        assert( bindings::size(x) >= 1+(n-1)*bindings::stride(x) );
        assert( bindings::size(y) >= 1+(n-1)*bindings::stride(y) );
...
//
template< typename Value >
struct largv_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
