...
#ifndef glas3_array_dense_array_container_dense_array_hpp
#define glas3_array_dense_array_container_dense_array_hpp
...
public:
	...
	dense_array ( value_type const& value )
	...
	dense_array ( std::initializer_list< value_type > const& data )
	...
	template <typename E>
	dense_array ( E const& data, typename std::enable_if<is<Container, E>::value>::type* = 0 )
	...
	// uninitialized dense_array constructor
	dense_array ( no_init, shape_type shape )
	: data_( )
	...
	dense_array ( value_type const& value, shape_type shape )
	: data_( )
	, shape_( boost::make_shared< shape_type >( std::move ( shape ) ) )
	, size_( 1 )
	{
	...
	// Copy constructor from std::initializer_list< value_type > -> deep copy
	dense_array ( std::initializer_list< value_type > const& data, shape_type shape )
	: data_( boost::make_shared_noinit< value_type[] >( data.size() ) )
	, shape_( boost::make_shared< shape_type >( std::move ( shape ) ) )
	, size_( 1 )
	{
	...
	// Copy constructor from Container -> deep copy
	template <typename E>
	dense_array ( E const& data, shape_type shape, typename std::enable_if<is<Container, E>::value>::type* = 0 )
	: data_( boost::make_shared_noinit< value_type[] >( data.size() ) )
	  , shape_( boost::make_shared< shape_type >( std::move ( shape ) ) )
	  , size_( 1 )
	  {
	...
	template <typename E>
	dense_array ( E const& that, typename std::enable_if<is<Array, E>::value>::type* = 0 )
...
public:
	...
	dense_array ( dense_array const& that )
...
public:
	...
	dense_array& operator= ( dense_array const& that ) {
	...
	// Move assignment
	dense_array& operator= ( dense_array&& that ) {
		assert( that.size() == size_ );
		//std::copy( that.data_, that.data_ + that.size_, data_ ) ; -> std::copy doesn't seem to work with boost::shared_ptr ??
		assign( *this, that ) ;
		return *this;
	...
	dense_array& operator= ( value_type const& value ) {
		assert( size_ == 1 );
		data_[0] = value ;
		return *this;
	}

	dense_array& operator= ( std::initializer_list< value_type > const& that ) {
		assert( that.size() == size_ );
	...
	template <typename E>
	typename std::enable_if< is<Container, E>::value, dense_array& >::type operator= ( E const& that ) {
		assert( that.size() == size_ );
		size_type i = 0 ;
		for ( auto x = that.begin(); x != that.end(); ++x, ++i) {
			data_[i] = *x ;
	...
	template<typename E>
	typename std::enable_if< is<Array, E>::value, dense_array& >::type operator= ( E const& that ) {
		assert( that.size() == size_ ) ;
		assign( *this, that ) ;
		return *this ;
	}
...
private:
	...
	dense_array ( boost::shared_ptr<value_type[]> data, boost::shared_ptr<shape_type> shape, size_type size )
	: data_( data )
	, shape_( shape )
	, size_( size )
	{}
...
public:
	dense_array shallow_copy () const {
		return dense_array( data_, shape_, size_ );
	}
...
template <typename V>
struct concept< dense_array<V>, typename std::enable_if< std::is_arithmetic<V>::value || boost::is_complex<V>::value >::type >
