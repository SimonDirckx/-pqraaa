...
class BlockHilbert: public SparseMatrix< T, LI > {
   ...
   protected:
	 ...
	 *
	 * @param build A series of sparse blocks with their Hilbert coordinates.
	 * @param input The input series of nonzeroes.
	 * @param blocki The row blocking size.
	 * @param blockj The column blocking size.
	 * @param depth Current recursion depth.
	 * @return Exit code: 0=failed split, 1=OK, 2=not split.
	 */
...
//		std::cout << "\t  storing block 2 of size " << (!dir?row_hi-rm-row_lo+1:row) << " by " << (dir?col_hi-cm-col_lo+1:col) << ", " << (input.size()-rows[rm]) << " nnz" << std::endl;
	...
	/** 
	 * Automatically performs recursive bisection on the input nonzeroes to
	 * achieve sparse blocking.
	 *
	 * @param build A series of sparse blocks with their Hilbert coordinates.
	 */
...
//			std::cout << "\t  --------------------" << std::endl;
	 ...
	 *
	 * @param input Original nonzero input.
	 * @return A series of sparse blocks (with Hilbert coordinates).
	 */
	 ...
	 * @param minexp The minimum amount of Hilbert coordinate expansions necessary for successful comparison during binary search.
   ...
   public:
	 ...
	 *  @param file The input sparse matrix file.
	 *  @param zero What element constitutes a matrix zero.
