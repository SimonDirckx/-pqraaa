...
#else
    LAPACK_SSYGVD( &itype, &jobz, &lapack_option< UpLo >::value, &n, a, &lda,
            b, &ldb, w, work, &lwork, iwork, &liwork, &info ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CHEGVD( &itype, &jobz, &lapack_option< UpLo >::value, &n, a, &lda,
            b, &ldb, w, work, &lwork, rwork, &lrwork, iwork, &liwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct hegvd_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct hegvd_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
//
template< typename MatrixA, typename MatrixB, typename VectorW >
inline typename std::enable_if<! detail::is_workspace< VectorW >::value,
        std::ptrdiff_t >::type
hegvd( const fortran_int_t itype, const char jobz, MatrixA& a,
        MatrixB& b, VectorW& w ) {
    return hegvd_impl< typename bindings::value_type<
