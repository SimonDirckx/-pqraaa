...
#else
    LAPACK_STGEVC( &lapack_option< Side >::value, &howmny, select, &n, s,
            &lds, p, &ldp, vl, &ldvl, vr, &ldvr, &mm, &m, work, &info ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CTGEVC( &lapack_option< Side >::value, &howmny, select, &n, s,
            &lds, p, &ldp, vl, &ldvl, vr, &ldvr, &mm, &m, work, rwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct tgevc_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const Side side, const char howmny,
        ...
        static_assert( (bindings::is_column_major< MatrixS >::value) );
        static_assert( (bindings::is_column_major< MatrixP >::value) );
        ...
        assert( bindings::size_column(s) >= 0 );
        assert( bindings::size_minor(p) == 1 ||
                bindings::stride_minor(p) == 1 );
        assert( bindings::size_minor(s) == 1 ||
        ...
        assert( bindings::stride_major(p) >= std::max< std::ptrdiff_t >(1,
                bindings::size_column(s)) );
        assert( bindings::stride_major(s) >= std::max< std::ptrdiff_t >(1,
                bindings::size_column(s)) );
...
//
template< typename Value >
struct tgevc_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
template< typename Side, typename VectorSELECT, typename MatrixS,
        typename MatrixP, typename MatrixVL, typename MatrixVR >
inline typename std::enable_if<! detail::is_workspace< MatrixVR >::value,
        std::ptrdiff_t >::type
tgevc( const Side side, const char howmny, const VectorSELECT& select,
        const MatrixS& s, const MatrixP& p, MatrixVL& vl, MatrixVR& vr,
