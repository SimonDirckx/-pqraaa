...
#else
    LAPACK_SGGEV( &jobvl, &jobvr, &n, a, &lda, b, &ldb, alphar, alphai, beta,
            vl, &ldvl, vr, &ldvr, work, &lwork, &info ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CGGEV( &jobvl, &jobvr, &n, a, &lda, b, &ldb, alpha, beta, vl,
            &ldvl, vr, &ldvr, work, &lwork, rwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct ggev_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct ggev_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
