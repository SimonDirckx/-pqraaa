...
#ifndef cork_matrix_valued_function_potential_theory_hpp
#define cork_matrix_valued_function_potential_theory_hpp
...
namespace CORK { namespace matrix_valued_function {
  ...
  decltype (auto) potential_theory ( NonlinearMatrix const& nep, Sigma const& sigma_domain, Xi const& xi_domain ) {
    return potential_theory<T>( nep, sigma_domain, xi_domain, matrix_valued_function::potential_theory_options< decltype(std::abs(T())) >() ) ;
  } // potential_theory()


  template <typename T, typename NonlinearMatrix, typename Sigma, typename Xi, typename Options>
  decltype (auto) potential_theory ( NonlinearMatrix const& nep, Sigma const& sigma_domain, Xi const& xi_domain, Options const& options ) {
    ...
    approximation::potential_theory_leja_bagby( sigma_points, xi_points, newton_basis ) ;
