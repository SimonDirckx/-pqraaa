...
#else
    LAPACK_STGSNA( &job, &howmny, select, &n, a, &lda, b, &ldb, vl, &ldvl, vr,
            &ldvr, s, dif, &mm, &m, work, &lwork, iwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct tgsna_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct tgsna_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
