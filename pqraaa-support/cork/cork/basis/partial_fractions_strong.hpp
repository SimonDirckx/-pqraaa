...
#ifndef cork_basis_partial_fractions_strong_hpp
#define cork_basis_partial_fractions_strong_hpp
...
namespace CORK { namespace basis {
  ...
  class partial_fractions_strong
  {
    public:
    ...
    public:
      explicit partial_fractions_strong( Poles poles, Weights weights )
      : poles_( poles )
      , weights_( weights )
      {
    ...
    public:
      ...
      void evaluate( ShiftValueType const& arg, FunctionValues values ) const {
        ...
        for (typename std::decay<FunctionValues>::type::size_type i=0; i<poles_.size(); ++i) {
          assert( arg!=poles_(i) ) ;
          values(i) = weights_(i) / (arg - poles_(i)) ;
        }
        values(poles_.size()) = 1. ;
      } // evaluate
  ...
  partial_fractions_strong<Poles&,Weights&> make_partial_fractions_strong_lvalue( Poles& poles, Weights& weights ) {
    return partial_fractions_strong<Poles&,Weights&>( poles, weights ) ;
  }

  template <typename Poles, typename Weights>
  decltype (auto) make_partial_fractions_strong_rvalue( Poles&& poles, Weights&& weights ) {
    return partial_fractions_strong<Poles&&, Weights&&>( poles, weights ) ;
  }

  template <typename Poles, typename Weights>
  partial_fractions_strong<Poles, Weights> make_partial_fractions_strong( Poles const& poles, Weights const& weights ) {
    return partial_fractions_strong<Poles, Weights>( poles, weights ) ;
  }
