...
#ifndef cork_basis4cork_partial_fractions_real_hpp
#define cork_basis4cork_partial_fractions_real_hpp

#include <cork/basis/partial_fractions_real.hpp>
#include <cork/basis4cork/partial_fractions_real_handle.hpp>
...
namespace CORK { namespace basis4CORK {
  ...
  class basis4CORK< basis::partial_fractions_real<Poles,Weights,I> >
  {
    public:
    ...
    public:
      ...
      using value_type = typename std::common_type< ValueType, decltype( std::abs(typename std::common_type< typename poles_type::value_type, typename weights_type::value_type>::type() ) ) >::type ;

    public:
      explicit basis4CORK( basis::partial_fractions_real<Poles,Weights,I> const& basis )
      : poles_( basis.poles() )
    ...
    public:
      ...
      void multiply_M( UM const& U, ZM Z ) const {
        ...
        for (int i=1; i<size(); ++i)
          if (poles_(i-1).imag()==0.) {
             Z(i-1,glas2::all()) = weights_(i-1).real() * U( 0, glas2::all() ) + poles_(i-1).real() * U( i, glas2::all() ) ;
          } else {
             Z(i-1,glas2::all()) = weights_(i-1).real() * U( 0, glas2::all() ) + poles_(i-1).real() * U( i, glas2::all() ) - poles_(i-1).imag() * U( i+1, glas2::all() ) ;
             Z(i,glas2::all()) = weights_(i-1).imag() * U( 0, glas2::all() ) + poles_(i-1).imag() * U( i, glas2::all() ) + poles_(i-1).real() * U( i+1, glas2::all() ) ;
             ++i ;
          }
      } // multiply_M()
    ...
    public:
      template <typename ValueType>
      using handle_type = partial_fractions_real_handle< ValueType, poles_type, weights_type, size_type > ;
