...
+-----------------------------------------------------------------------*/ 
	 ...
	 for (j=0; j<gmat->nzcount[jnod]; j++){	
	   jcol = rowj[j];
	   if (iord[jcol] == -1) 
	     zadd2com(&nback, jcol, iord, riord);
	 }
       }
       gmat = matT; 	
     }
