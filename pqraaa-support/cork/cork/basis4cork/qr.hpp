...
#include <cork/basis4cork/qr_handle.hpp>
...
namespace CORK { namespace basis4CORK {
  ...
  class basis4CORK< basis::qr<TT> >
  {
    public:
      typedef I                     size_type ;

      template< typename T>
      using handle_type = qr_handle<TT, T, size_type> ;
    ...
    public:
      ...
      typename std::enable_if< glas2::is< glas2::DenseMatrix, AA >::value >::type fill_M( AA A ) {
      ...
      //Basis4CORK
      template <typename AA>
      typename std::enable_if< glas2::is< glas2::DenseMatrix, AA >::value >::type fill_N( AA A ) {
        assert( A.num_rows() == size()-1 ) ;
        assert( A.num_columns() == size() ) ;
        fill( A, 0.0 ) ;
