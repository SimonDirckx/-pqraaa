...
#include <cork/linearization/barycentric_rational_real.hpp>
...
namespace CORK {
  ...
  */
  ...
  decltype (auto) raaa_cork( NEP const& problem, Domain const& aaa_domain, EigenvalueSelector const& eig_selector, Options const& options ) {
    ...
    auto PEP = make_matrix_polynomial_with_shadow( CORK::matrix_valued_function::sv_raaa( nep_krylov, nep_aaa, aaa_domain, norm_est, options ), nep_krylov ) ;
