...
#else
    LAPACK_STBCON( &norm, &lapack_option< UpLo >::value, &lapack_option<
            Diag >::value, &n, &kd, ab, &ldab, &rcond, work, iwork, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CTBCON( &norm, &lapack_option< UpLo >::value, &lapack_option<
            Diag >::value, &n, &kd, ab, &ldab, &rcond, work, rwork, &info ,1 ,1 ,1 );
#endif
    return info;
}
...
struct tbcon_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
...
//
template< typename Value >
struct tbcon_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
