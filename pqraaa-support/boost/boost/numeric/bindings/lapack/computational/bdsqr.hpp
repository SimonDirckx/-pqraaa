...
struct bdsqr_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const char uplo, const fortran_int_t n,
        ...
        assert( bindings::size_column(vt) >= 0 );
        ...
        assert( bindings::size_row(u) >= 0 );
        assert( bindings::stride_major(u) >= std::max< std::ptrdiff_t >(1,
                bindings::size_row(u)) );
        assert( n >= 0 );
...
//
template< typename Value >
struct bdsqr_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
