...
#else
    LAPACK_SSTEGR( &jobz, &range, &n, d, e, &vl, &vu, &il, &iu, &abstol, &m,
            w, z, &ldz, isuppz, work, &lwork, iwork, &liwork, &info ,1 ,1 );
#endif
    return info;
}
...
struct stegr_impl< Value, typename std::enable_if< is_real< Value >::value >::type > {
    ...
    static std::ptrdiff_t invoke( const char jobz, const char range,
        ...
        static_assert( (bindings::is_mutable< VectorISUPPZ >::value) );
...
//
template< typename Value >
struct stegr_impl< Value, typename std::enable_if< is_complex< Value >::value >::type > {
...
template< typename VectorD, typename VectorE, typename VectorW,
        typename MatrixZ, typename VectorISUPPZ >
inline typename std::enable_if<! detail::is_workspace< VectorISUPPZ >::value,
        std::ptrdiff_t >::type
stegr( const char jobz, const char range, const fortran_int_t n,
        VectorD& d, VectorE& e, const typename remove_imaginary<
        typename bindings::value_type< MatrixZ >::type >::type vl,
...
stegr( const char jobz, const char range, const fortran_int_t n,
        ...
        const VectorISUPPZ& isuppz, Workspace work ) {
    return stegr_impl< typename bindings::value_type<
            MatrixZ >::type >::invoke( jobz, range, n, d, e, vl, vu, il, iu,
            abstol, m, w, z, isuppz, work );
}
