...
#else
    LAPACK_SLANSY( &norm, &lapack_option< UpLo >::value, &n, a, &lda, work ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_DLANSY( &norm, &lapack_option< UpLo >::value, &n, a, &lda, work ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_CLANSY( &norm, &lapack_option< UpLo >::value, &n, a, &lda, work ,1 ,1 );
#endif
    return info;
}
...
#else
    LAPACK_ZLANSY( &norm, &lapack_option< UpLo >::value, &n, a, &lda, work ,1 ,1 );
#endif
    return info;
}
...
struct lansy_impl {
    ...
    static std::ptrdiff_t invoke( const char norm, const MatrixA& a,
        ...
        assert( bindings::stride_major(a) >= std::max<
