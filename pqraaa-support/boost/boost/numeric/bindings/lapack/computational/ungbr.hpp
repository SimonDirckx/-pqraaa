...
#else
    LAPACK_CUNGBR( &vect, &m, &n, &k, a, &lda, tau, work, &lwork, &info ,1 );
#endif
    return info;
}
...
#else
    LAPACK_ZUNGBR( &vect, &m, &n, &k, a, &lda, tau, work, &lwork, &info ,1 );
#endif
    return info;
}
...
struct ungbr_impl {
    ...
    static std::ptrdiff_t invoke( const char vect, const fortran_int_t m,
        ...
        assert( bindings::stride_major(a) >= m );
        assert( k >= 0 );
        assert( m >= 0 );
...
// Calls to these functions are passed to the ungbr_impl classes. In the 
